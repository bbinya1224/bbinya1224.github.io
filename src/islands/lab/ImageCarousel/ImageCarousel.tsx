...
export const ImageCarousel = ({
  ...
  const renderThumbnail = (
    ...
    <div
      className='group relative aspect-[16/9] w-full cursor-pointer overflow-hidden bg-surface'
  ...
  return (
    <>
                      ...
                      <CarouselItem key={`modal-${index}`}>
                        <div className='relative flex h-[80vh] w-full items-center justify-center'>
                          <img
                            ...
                            loading={
                              index === currentSlide - 1 ? 'eager' : 'lazy'
                            }
                            decoding='async'
                          />
                        </div>
      ...
      {images.length === 1 ? (
        <div className='not-prose relative mx-auto my-8 w-full max-w-4xl overflow-hidden rounded-lg border border-line'>
            ...
            <CarouselContent>
              {images.map((image, index) => (
                <CarouselItem key={`thumbnail-${index}`}>
                  <div className='relative overflow-hidden rounded-lg border border-line shadow-sm'>
                    {renderThumbnail(
                      image,
                      index,
                      index === 0 ? 'eager' : 'lazy',
