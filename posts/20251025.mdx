...
## 방법 2: Suspense + mounted 함께 사용하기

<aside data-type="warning">

**중요: useSearchParams() 사용 시 Suspense 필수!**

Next.js에서 `useSearchParams()`를 사용하는 클라이언트 컴포넌트는 **반드시 Suspense로 감싸야 합니다**.  
...
따라서 `useSearchParams()`를 사용하는 컴포넌트는 **Suspense와 mounted 상태를 함께 사용**해야 합니다.

```tsx
// Tag.tsx - Suspense는 Next.js 요구사항 충족용
<Suspense fallback={<TagSkeleton />}>
  <TagList />
</Suspense>

// TagList.tsx - mounted는 클라이언트 사이드 스켈레톤 표시용
"use client";

const TagList = () => {
  const [tags] = useAtom(tagsAtom);
  const [mounted, setMounted] = useState(false);
  const searchParams = useSearchParams(); // 이것 때문에 Suspense 필요!
...
- **Suspense**: Next.js 빌드 요구사항 충족 + SSR 중 스켈레톤 표시
- **mounted 상태**: 클라이언트 사이드 hydration 중 스켈레톤 표시 + hydration 불일치 방지

`useSearchParams()`**를 사용하지 않는 컴포넌트라면?**

Header처럼 `useSearchParams()`를 사용하지 않는 컴포넌트는 Suspense 없이 mounted 상태만 사용해도 됩니다.
...
- **클라이언트 컴포넌트 + useSearchParams() 사용**
  → `Suspense` + `mounted` 상태 **함께 사용** (Next.js 필수 요구사항)

- **클라이언트 컴포넌트 + 일반 hooks만 사용**
  → `mounted` 상태로 스켈레톤 처리 (Suspense 불필요)

- **큰 클라이언트 컴포넌트 (코드 스플리팅 필요)**
  → `React.lazy()` + `Suspense`

<aside data-type="info">
1. **Suspense 단독으로는 클라이언트 컴포넌트의 스켈레톤을 보여줄 수 없습니다**
   - 클라이언트 컴포넌트는 동기적으로 실행되어 fallback이 트리거되지 않음

2. **useSearchParams()를 사용하면 Suspense는 필수입니다**
   - Next.js 빌드 요구사항
   - 하지만 클라이언트 사이드 스켈레톤을 위해 mounted 상태도 필요

3. **mounted 상태는 hydration 문제도 해결합니다**
   - 서버/클라이언트 렌더링 불일치 방지
   - 클라이언트 전용 기능(테마, 검색 파라미터) 처리
</aside>

## 최종 코드 구조

```tsx
// Tag.tsx - useSearchParams 사용하므로 Suspense 필수
...
// TagList.tsx - Suspense + mounted 상태 함께 사용
"use client";

const TagList = () => {
  const [tags] = useAtom(tagsAtom);
  const [mounted, setMounted] = useState(false);
  const searchParams = useSearchParams(); // Suspense 필수!
