...
그런데 막상 적용해보니 문제가 생기더라고요.

입력 필드에 타이핑할 때마다 화면 전체가 리렌더링되면서 버벅거리기 시작한 거예요.  
특히 무거운 컴포넌트들이 같이 있는 페이지에서는 체감이 심했습니다.

"이거... 제어 컴포넌트 문제인가?" 싶어서 비제어 컴포넌트로 바꿔봤더니 버벅임은 사라졌는데,  
이번엔 실시간 유효성 검증이 안 되더라고요.

---

## 배경: Radix UI와 비제어 컴포넌트

사실 제가 이 주제에 관심을 갖게 된 건 **Radix UI**를 사용하면서 인데요,
...
const UncontrolledPositiveInput: React.FC = () => {
...
## 성능 최적화: useTransition으로 제어 컴포넌트 개선하기

제어 컴포넌트의 가장 큰 단점은 **입력할 때마다 리렌더링이 발생**한다는 것 입니다.  
특히 무거운 컴포넌트 트리를 가진 경우, 입력이 버벅거리는 문제가 생기게 됩니다.

React의 `useTransition` 훅을 사용하면 어떨까 했어요.

**useTransition의 장점:**
- **입력 반응성 유지**: 사용자 타이핑은 즉시 반영됩니다.
- **백그라운드 업데이트**: 무거운 State 업데이트는 우선순위를 낮춰 처리합니다.
- **UI 블로킹 방지**: 브라우저가 입력을 먼저 처리하고, 여유가 있을 때 State를 업데이트합니다.
- **일관성 보장**: 타이머 없이 React의 동시성 기능으로 자연스럽게 처리됩니다.
...
const OptimizedControlledInput: React.FC = () => {
  const [displayValue, setDisplayValue] = useState<string>('');
  const [lastValidValue, setLastValidValue] = useState<string>('');

  // useTransition 훅
  // isPending: 전환 중(transition) 상태를 나타냄
  // startTransition: 낮은 우선순위로 State 업데이트를 실행하는 함수
  const [isPending, startTransition] = useTransition();

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newValue = e.target.value;

    // startTransition으로 감싸진 State 업데이트는
    // "긴급하지 않은 업데이트"로 분류돼요
    startTransition(() => {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'green', fontSize: '12px' }}>
        ✅ 입력 반응성은 유지하면서 무거운 업데이트는 백그라운드로
      </p>
    </div>
  );
};
...
React의 **Concurrent Rendering** 기능 덕분에 `useTransition`이 가능합니다.
...
3. **입력 반응성 유지**
   - `displayValue` State가 늦게 업데이트되더라도, 네이티브 input 요소는 즉시 반응
   - 사용자는 지연을 느끼지 못하게 됩니다.


```tsx
const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  startTransition(() => {
    setDisplayValue(e.target.value); // 우선순위만 낮춤, 지연 없음
  });
};
// 장점: 입력은 즉시 반영, State는 여유가 있을 때 업데이트
```

> **useTransition**: 우선순위 조정 → 입력은 즉시 반영, State는 자연스럽게 동기화
